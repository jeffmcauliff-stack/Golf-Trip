...
const COURSES = [
  {
    id: "twin", name: "Twin Hills", designer: "Donald Sechrest · Twin Hills C.C., Joplin MO",
  ...
  },
  {
    ...
    type: "Championship", championship: true,
    matchPlay: { teamA: ["Jeff", "Kevin"], teamB: ["Carson", "Brian"], stake: 10 },
...
const K_ROUNDS = "bigcedar_rounds", K_PARS = "bigcedar_pars", K_STAKES = "bigcedar_stakes";
...
function tiebreakResults(rounds) {
  return tiebreakOrder().map((c) => {
    ...
    const declared = (rs.find((r) => r.puttOffWinner) || {}).puttOffWinner || null;
    const isTie = played && low.length > 1;
    let winner = played && low.length === 1 ? low[0].name : null;
    let puttOff = false;
    if (isTie && declared && low.some((x) => x.name === declared)) { winner = declared; puttOff = true; }
    return { courseId: c.id, name: c.name, played, winner, tie: isTie && !winner, puttOff, tiedNames: low.map((x) => x.name), totals: tot, low: min };
  });
}
...
function ghostRace(rounds, rate) {
  rate = rate == null ? 1 : Number(rate);
  const houseName = (PLAYERS.find((p) => !Object.keys(PRIOR).some((cid) => PRIOR[cid][p.name])) || {}).name || null;
  const direct = {}; PLAYERS.forEach((p) => (direct[p.name] = 0));
  const pools = {};
  const byCourse = [];
  Object.keys(PRIOR).forEach((cid) => {
    const c = courseById(cid); if (!c || c.tiebreaker || cid === "twin") return;
    const sc = scoringHoles(cid);
    const rd = rounds.find((r) => r.courseId === cid);
    const pick = rd && rd.ghostPick && PRIOR[cid][rd.ghostPick] ? rd.ghostPick : null;
    const dormant = !pick;
    const poolId = settlingTiebreaker(cid);
    const poolCourse = poolId ? courseById(poolId) : null;
    const rows = [];
    Object.keys(PRIOR[cid]).forEach((name) => {
      const ghost = PRIOR[cid][name];
      const live = (rd && rd.holeScores && rd.holeScores[name]) || [];
      let liveSum = 0, ghostSum = 0, played = 0, ghostHoles = 0;
      for (let i = 0; i < sc; i++) { if (isNum(ghost[i])) ghostHoles++; if (isNum(live[i]) && isNum(ghost[i])) { liveSum += Number(live[i]); ghostSum += Number(ghost[i]); played++; } }
      const diff = liveSum - ghostSum;
      const complete = played > 0 && played === ghostHoles;
      const beatAmt = played > 0 && diff < 0 ? (-diff) * rate : 0;
      const loseAmt = played > 0 && diff > 0 ? diff * rate : (complete && diff === 0 ? rate : 0);
      const status = played === 0 ? "none" : diff < 0 ? "beat" : diff > 0 ? "behind" : complete ? "tie" : "even";
      let amt = 0;
      if (!dormant) {
        if (name === pick) {
          if (beatAmt > 0) { amt = beatAmt; direct[name] += beatAmt; if (houseName) direct[houseName] -= beatAmt; }
          else if (loseAmt > 0) { amt = -loseAmt; direct[name] -= loseAmt; if (houseName) direct[houseName] += loseAmt; }
        } else if (loseAmt > 0) {
          amt = -loseAmt;
          if (poolId) { pools[poolId] = pools[poolId] || { by: {}, total: 0 }; pools[poolId].by[name] = (pools[poolId].by[name] || 0) + loseAmt; pools[poolId].total += loseAmt; }
        }
      }
      rows.push({ name, ghost: ghostSum, live: liveSum, played, complete, diff, isPick: name === pick, amt, status });
    });
    byCourse.push({ courseId: cid, name: c.name, pick, dormant, poolId, poolName: poolCourse ? poolCourse.name : null, rows });
  });
  byCourse.sort((a, b) => whenKey(courseById(a.courseId)) - whenKey(courseById(b.courseId)));
  return { houseName, rate, direct, pools, byCourse };
}

function seasonMoney(rounds, overrides, stakes) {
  const t = {}; PLAYERS.forEach((p) => (t[p.name] = { skins: 0, nassau: 0, greenies: 0, match: 0, tiebreak: 0, ghost: 0, greenieCount: 0, total: 0 }));
  ...
  const gr = ghostRace(rounds, stakes && stakes.ghost != null ? stakes.ghost : 1);
  PLAYERS.forEach((p) => (t[p.name].ghost = gr.direct[p.name]));
  tbs.forEach((tb) => { if (!tb.winner) return; const gp = gr.pools[tb.courseId]; if (!gp) return; Object.keys(gp.by).forEach((n) => (t[n].ghost -= gp.by[n])); t[tb.winner].tiebreak += gp.total; });
  PLAYERS.forEach((p) => (t[p.name].total = t[p.name].skins + t[p.name].nassau + t[p.name].greenies + t[p.name].match + t[p.name].tiebreak + t[p.name].ghost));
  return t;
}
...
/* ============ DEMO DATA ============ */
const DEMO = true; // demo build: seeds random scores, does not read/write storage
function makeDemoRounds() {
  const rnd = (a, b) => a + Math.floor(Math.random() * (b - a + 1));
  const scoreFor = (par) => par + rnd(-1, 2); // birdie .. double bogey
  const build = (courseId, date, idx) => {
    const course = courseById(courseId);
    const pars = course.defaultPars;
    const sc = scoringHoles(courseId);
    const holeScores = {};
    PLAYERS.forEach((p) => { holeScores[p.name] = pars.map((par) => scoreFor(par)); });
    const greenies = {};
    if (course.championship) {
      pars.forEach((par, i) => {
        if (((par === 3 && i < sc) || isTigerHole(courseId, i)) && Math.random() < 0.75) greenies[i] = PLAYERS[rnd(0, 3)].name;
      });
    }
    return { id: 1717000000000 + idx, courseId, date, holeScores, greenies };
  };
  return COURSES.map((c, i) => build(c.id, c.teeTime?.date || `2026-06-0${i + 1}`, i + 1));
}

/* ============ app ============ */
export default function App() {
  const [tab, setTab] = useState("play");
  const [rounds, setRounds] = useState(() => (DEMO ? makeDemoRounds() : []));
  const [pars, setPars] = useState({});
  const [stakes, setStakes] = useState(DEFAULT_STAKES);
  const [draft, setDraft] = useState(null);
  const [isAdmin, setIsAdmin] = useState(() => (typeof window !== "undefined" && window.BC_AUTH ? !!window.BC_AUTH.isAdmin : true));
  useEffect(() => { if (typeof window === "undefined" || !window.BC_AUTH) return; const h = () => setIsAdmin(!!window.BC_AUTH.isAdmin); window.addEventListener("bc-admin", h); h(); return () => window.removeEventListener("bc-admin", h); }, []);
  ...
  const save = async (key, value, setter) => { setter(value); if (DEMO) return; try { const ok = await window.storage.set(key, JSON.stringify(value)); setSaveError(!ok); } catch (e) { setSaveError(true); } };
    ...
    <div style={{ background: C.parchment, minHeight: "100vh", color: C.ink, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <main className="mx-auto px-4 pb-20 pt-4" style={{ maxWidth: 820 }}>
        {loading ? <p className="py-12 text-center" style={{ color: C.muted }}>Loading the scorecard…</p>
          : showTab === "play" ? <Play pars={pars} rounds={sorted} stakes={stakes} isAdmin={isAdmin} onLive={(r) => { const ex = rounds.find((x) => x.id === r.id) || rounds.find((x) => x.courseId === r.courseId) || {}; save(K_ROUNDS, upsertRound(rounds, { ...ex, ...r }), setRounds); }} />
          : showTab === "rounds" ? <Rounds rounds={sorted} pars={pars} stakes={stakes} isAdmin={isAdmin} onUpdate={(id, r) => save(K_ROUNDS, rounds.map((x) => (x.id === id ? { ...x, ...r, id } : x)), setRounds)} onDelete={(id) => save(K_ROUNDS, rounds.filter((x) => x.id !== id), setRounds)} />
          : showTab === "ghost" ? <Ghost rounds={sorted} stakes={stakes} isAdmin={isAdmin} onPick={(cid, name) => { const ex = rounds.find((x) => x.courseId === cid); let upd; if (ex) { upd = { ...ex }; if (name) upd.ghostPick = name; else delete upd.ghostPick; } else { if (!name) return; const c = courseById(cid); upd = { id: "r-" + cid, courseId: cid, date: (c.teeTime && c.teeTime.date) || today(), holeScores: {}, greenies: {}, ghostPick: name }; } save(K_ROUNDS, upsertRound(rounds, upd), setRounds); }} />
          : showTab === "stats" ? <Stats rounds={sorted} pars={pars} />
          : showTab === "trip" ? <Trip rounds={sorted} pars={pars} stakes={stakes} isAdmin={isAdmin} setStakes={(v) => save(K_STAKES, v, setStakes)} onClear={() => save(K_ROUNDS, [], setRounds)} onPuttOff={(cid, w) => { const r = rounds.find((x) => x.courseId === cid); if (!r) return; const u = { ...r }; if (w) u.puttOffWinner = w; else delete u.puttOffWinner; save(K_ROUNDS, upsertRound(rounds, u), setRounds); }} />
          : showTab === "schedule" ? <Schedule />
          : showTab === "history" ? <History />
          : showTab === "stakes" ? <Stakes stakes={stakes} isAdmin={isAdmin} setStakes={(v) => save(K_STAKES, v, setStakes)} />
          : <Courses pars={pars} isAdmin={isAdmin} setPars={(v) => save(K_PARS, v, setPars)} />}
      </main>
    </div>
  );
}
...
function InstallHint() {
  ...
  useEffect(() => {
    ...
    const p = /iPhone|iPad|iPod/i.test(ua) ? "ios" : (/Android/i.test(ua) ? "android" : "other");
    if (p === "other") return;
    setPlat(p);
    (async () => { try { const d = await window.storage.get("bigcedar_hideinstall"); if (d && d.value === "1") return; } catch (e) {} setShow(true); })();
  }, []);
  if (!show) return null;
  const dismiss = () => { setShow(false); try { window.storage.set("bigcedar_hideinstall", "1"); } catch (e) {} };
  const steps = plat === "ios"
    ? <>In Safari, tap <b>Share</b> <span style={{ opacity: 0.7 }}>(box with an up-arrow)</span>, then <b>Add to Home Screen</b>.</>
    : <>In Chrome, tap the <b>⋮</b> menu, then <b>Add to Home screen</b>.</>;
  return (
    <div className="mx-auto px-4 pt-3" style={{ maxWidth: 820 }}>
      <div className="flex items-start gap-3 rounded-lg px-3 py-2" style={{ background: "rgba(201,162,75,0.14)", border: `1px solid ${C.gold}` }}>
        <span style={{ fontSize: 18, lineHeight: 1.2 }}>📲</span>
        <div className="flex-1 text-xs">
          <div className="font-bold" style={{ color: C.pine }}>Add to your Home Screen</div>
          <div className="mt-0.5" style={{ color: C.muted }}>{steps} It opens full-screen like an app and keeps updating live.</div>
        </div>
        <button onClick={dismiss} aria-label="Dismiss" className="font-bold" style={{ color: C.muted, fontSize: 18, lineHeight: 1 }}>×</button>
      </div>
    </div>
  );
}
...
function Header() {
  return (
    <header style={{ background: C.pine, color: C.parchment }}>
      <div className="mx-auto px-4" style={{ maxWidth: 820, paddingTop: "calc(env(safe-area-inset-top) + 18px)", paddingBottom: 20 }}>
        <div className="flex items-center gap-3">
          <img src="./icon-192.png" alt="Big Cedar Golf" onError={(e) => { e.currentTarget.style.display = "none"; }} style={{ width: 56, height: 56, flexShrink: 0 }} />
          <div>
            <p className="text-xs" style={{ color: C.gold, letterSpacing: "0.25em" }}>OZARKS GOLF TRIP{DEMO ? " · DEMO DATA" : ""}</p>
            <h1 style={{ fontFamily: serif, fontSize: 30, fontWeight: 700, lineHeight: 1.05 }}>Big Cedar Lodge</h1>
          </div>
        </div>
...
function Tabs({ tab, setTab, isAdmin }) {
  const base = [["play", "Play"], ["rounds", "Rounds"], ["trip", "Leaderboard"], ["ghost", "Ghost"], ["stats", "Stats"], ["schedule", "Schedule"], ["courses", "Courses"], ["history", "History"], ["stakes", "Stakes"]];
  const items = base;
  const auth = typeof window !== "undefined" ? window.BC_AUTH : null;
  return (
    <nav style={{ background: C.card, borderBottom: `1px solid ${C.line}`, position: "sticky", top: 0, zIndex: 5 }}>
      <div className="mx-auto px-4 py-3" style={{ maxWidth: 820 }}>
        <div className="flex items-center gap-2">
          ...
          {auth && (isAdmin
            ? <button onClick={() => auth.signOut()} className="rounded-lg px-3 py-2 text-xs font-semibold" style={{ border: `1px solid ${C.fairway}`, color: C.fairway, background: "#fff", whiteSpace: "nowrap" }}>Admin ✓</button>
            : <button onClick={() => auth.signIn()} className="rounded-lg px-3 py-2 text-xs font-semibold" style={{ border: `1px solid ${C.line}`, color: C.muted, background: "#fff", whiteSpace: "nowrap" }}>Admin sign-in</button>)}
        </div>
      </div>
    </nav>
  );
...
function Scorecard({ holePars, hs, setCell, subtotal, editable, readScores, courseId }) {
  ...
  const course = courseById(courseId);
  const mp = course && course.matchPlay ? course.matchPlay : null;
  const orderedPlayers = mp ? [...mp.teamA, ...mp.teamB].map((n) => PLAYERS.find((p) => p.name === n)).filter(Boolean) : PLAYERS;
  const boundaryName = mp && mp.teamB[0];
  const teamBorder = (p) => (p && p.name === boundaryName ? `2px solid ${C.gold}` : undefined);
  const bestBall = (names, i) => { const s = names.map((n) => getV(n, i)).filter(isNum).map(Number); return s.length ? Math.min(...s) : null; };
  const matchStat = (() => {
    if (!mp) return null;
    let up = 0, played = 0, closeUp = 0, closeRem = -1;
    for (let i = 0; i < scoring; i++) {
      const a = bestBall(mp.teamA, i), b = bestBall(mp.teamB, i);
      if (a == null || b == null) continue;
      played++;
      if (a < b) up++; else if (b < a) up--;
      const rem = scoring - played;
      if (closeRem < 0 && Math.abs(up) > rem) { closeUp = up; closeRem = rem; }
    }
    const remaining = scoring - played;
    const decided = closeRem >= 0 || (played === scoring && played > 0);
    const dormie = closeRem < 0 && remaining > 0 && up !== 0 && Math.abs(up) === remaining;
    return { up, played, remaining, decided, dormie, closeUp, closeRem };
  })();
  const sideLabel = (x) => (x === 0 ? "All square" : `${Math.abs(x)} ${x > 0 ? "UP" : "DN"}`);
  const matchResult = (() => {
    if (!mp || !matchStat || !matchStat.decided) return null;
    if (matchStat.closeRem >= 0) {
      const margin = Math.abs(matchStat.closeUp);
      const notation = matchStat.closeRem === 0 ? `${margin} up` : `${margin}&${matchStat.closeRem}`;
      return { winner: (matchStat.closeUp > 0 ? mp.teamA : mp.teamB).join(" & "), notation };
    }
    if (matchStat.up === 0) return { halved: true };
    return { winner: (matchStat.up > 0 ? mp.teamA : mp.teamB).join(" & "), notation: `${Math.abs(matchStat.up)} up` };
  })();

  const scoreCell = (p, i, par) => {
    const v = getV(p.name, i); const tp = isNum(v) ? Number(v) - par : null;
    return (
      <td key={p.name} className="px-0.5 py-1 text-center" style={{ borderLeft: teamBorder(p) }}>
        {editable
          ? (() => {
  ...
  const bbHoleCell = (names, i, par, key) => { const val = bestBall(names, i); const tp = val != null ? val - par : null; return <td key={key} className="px-0.5 py-1 text-center text-sm font-bold" style={{ background: "rgba(201,162,75,0.14)", color: val != null ? parColor(tp) : C.muted }}>{val != null ? val : "·"}</td>; };
  const bbSum = (names, from, to) => { let t = 0, any = false; for (let i = from; i < to; i++) { const v = bestBall(names, i); if (v != null) { t += v; any = true; } } return any ? t : ""; };
  const rowEl = (i, par, bonus) => (
    <tr key={i} style={{ borderTop: `1px solid ${C.line}`, background: bonus ? "rgba(201,162,75,0.14)" : "transparent" }}>
      ...
      {mp ? (
        <>
          {orderedPlayers.slice(0, mp.teamA.length).map((p) => scoreCell(p, i, par))}
          {bbHoleCell(mp.teamA, i, par, "bbA")}
          {orderedPlayers.slice(mp.teamA.length).map((p) => scoreCell(p, i, par))}
          {bbHoleCell(mp.teamB, i, par, "bbB")}
        </>
      ) : (
        orderedPlayers.map((p) => scoreCell(p, i, par))
      )}
    </tr>
  );
  const subRowEl = (from, to, label) => (
    <tr key={label} style={{ background: "rgba(46,94,58,0.06)" }}>
      ...
      {mp ? (
        <>
          {orderedPlayers.slice(0, mp.teamA.length).map((p) => <td key={p.name} className="px-0.5 py-1 text-center text-xs font-bold" style={{ borderLeft: teamBorder(p) }}>{subtotal(p.name, from, to) || ""}</td>)}
          <td key="bbA" className="px-0.5 py-1 text-center text-xs font-bold" style={{ background: "rgba(201,162,75,0.14)" }}>{bbSum(mp.teamA, from, to)}</td>
          {orderedPlayers.slice(mp.teamA.length).map((p) => <td key={p.name} className="px-0.5 py-1 text-center text-xs font-bold" style={{ borderLeft: teamBorder(p) }}>{subtotal(p.name, from, to) || ""}</td>)}
          <td key="bbB" className="px-0.5 py-1 text-center text-xs font-bold" style={{ background: "rgba(201,162,75,0.14)" }}>{bbSum(mp.teamB, from, to)}</td>
        </>
      ) : (
        orderedPlayers.map((p) => <td key={p.name} className="px-0.5 py-1 text-center text-xs font-bold" style={{ borderLeft: teamBorder(p) }}>{subtotal(p.name, from, to) || ""}</td>)
      )}
    </tr>
  );
  ...
  return (
    <>
      {matchStat && (
        <div className="mb-2" style={{ minWidth: 300 }}>
          <div className="flex items-stretch gap-2 text-xs">
            <div className="flex-1 rounded-lg px-3 py-2" style={{ border: `1px solid ${C.line}`, background: "rgba(46,94,58,0.06)" }}>
              <div className="font-bold" style={{ color: C.ink }}>{mp.teamA.join(" & ")}</div>
              <div style={{ fontWeight: 700, color: matchStat.up > 0 ? C.fairway : matchStat.up < 0 ? C.over : C.muted }}>{sideLabel(matchStat.up)}</div>
            </div>
            <div className="flex items-center whitespace-nowrap" style={{ color: matchStat.dormie ? C.gold : C.muted, fontWeight: matchStat.dormie ? 700 : 400 }}>{matchStat.played === 0 ? "not started" : matchStat.decided ? "Final" : matchStat.dormie ? "DORMIE" : `${matchStat.remaining} to play`}</div>
            <div className="flex-1 rounded-lg px-3 py-2 text-right" style={{ border: `1px solid ${C.line}`, background: "rgba(46,94,58,0.06)" }}>
              <div className="font-bold" style={{ color: C.ink }}>{mp.teamB.join(" & ")}</div>
              <div style={{ fontWeight: 700, color: -matchStat.up > 0 ? C.fairway : -matchStat.up < 0 ? C.over : C.muted }}>{sideLabel(-matchStat.up)}</div>
            </div>
          </div>
          {matchResult && (
            <div className="mt-2 rounded-lg px-3 py-2 text-center text-sm font-bold" style={matchResult.halved ? { background: "rgba(111,106,94,0.12)", color: C.muted } : { background: "rgba(201,162,75,0.18)", border: `1px solid ${C.gold}`, color: C.pine }}>
              {matchResult.halved ? "Match halved" : `🏆 ${matchResult.winner} won ${matchResult.notation}`}
            </div>
          )}
        </div>
      )}
    <table ref={tableRef} style={{ borderCollapse: "collapse", width: "100%", minWidth: 300 }}>
      <thead>
        <tr style={{ background: C.pine, color: C.parchment }}>
          ...
          {mp ? (
            <>
              {orderedPlayers.slice(0, mp.teamA.length).map((p) => <th key={p.name} className="px-1 py-1 text-xs" style={{ color: "#fff", borderLeft: teamBorder(p) }}>{p.name}</th>)}
              <th key="bbA" className="px-1 py-1 text-xs" style={{ color: C.gold }}>Best</th>
              {orderedPlayers.slice(mp.teamA.length).map((p) => <th key={p.name} className="px-1 py-1 text-xs" style={{ color: "#fff", borderLeft: teamBorder(p) }}>{p.name}</th>)}
              <th key="bbB" className="px-1 py-1 text-xs" style={{ color: C.gold }}>Best</th>
            </>
          ) : (
            orderedPlayers.map((p) => <th key={p.name} className="px-1 py-1 text-xs" style={{ color: "#fff", borderLeft: teamBorder(p) }}>{p.name}</th>)
          )}
        </tr>
      </thead>
      <tbody>
        ...
        {has18 && (
          <tr style={{ background: C.pine, color: C.parchment }}>
            ...
            {mp ? (
              <>
                {orderedPlayers.slice(0, mp.teamA.length).map((p) => <td key={p.name} className="px-0.5 py-1 text-center text-xs font-bold" style={{ borderLeft: teamBorder(p) }}>{subtotal(p.name, 0, scoring) || ""}</td>)}
                <td key="bbA" className="px-0.5 py-1 text-center text-xs font-bold">{bbSum(mp.teamA, 0, scoring)}</td>
                {orderedPlayers.slice(mp.teamA.length).map((p) => <td key={p.name} className="px-0.5 py-1 text-center text-xs font-bold" style={{ borderLeft: teamBorder(p) }}>{subtotal(p.name, 0, scoring) || ""}</td>)}
                <td key="bbB" className="px-0.5 py-1 text-center text-xs font-bold">{bbSum(mp.teamB, 0, scoring)}</td>
              </>
            ) : (
              orderedPlayers.map((p) => <td key={p.name} className="px-0.5 py-1 text-center text-xs font-bold" style={{ borderLeft: teamBorder(p) }}>{subtotal(p.name, 0, scoring) || ""}</td>)
            )}
          </tr>
        )}
...
/* ============ PLAY ============ */
function RoundEditor({ pars, stakes, rounds, onLive, initial, onSave, onCancel }) {
  const startId = initial ? initial.courseId : COURSES[0].id;
  const defaultDate = (id) => courseById(id).teeTime?.date || today();
  const existingFor = (id) => (rounds || []).find((r) => r.courseId === id);
  const fromRound = (r, id) => { const len = getPars(id, pars).length; const o = {}; PLAYERS.forEach((p) => { o[p.name] = Array(len).fill("").map((_, i) => { const v = r?.holeScores?.[p.name]?.[i]; return isNum(v) ? String(v) : ""; }); }); return o; };
  const blank = (id) => { const o = {}; PLAYERS.forEach((p) => (o[p.name] = Array(getPars(id, pars).length).fill(""))); return o; };
  const seed0 = initial ? null : existingFor(startId);
  const [courseId, setCourseId] = useState(startId);
  const [date, setDate] = useState(() => (initial ? initial.date : (seed0 ? seed0.date : defaultDate(startId))));
  const course = courseById(courseId);
  const holePars = getPars(courseId, pars);
  const sc = scoringHoles(courseId);
  const stk = stakes || DEFAULT_STAKES;

  const [hs, setHs] = useState(() => (initial ? fromRound(initial, startId) : (seed0 ? fromRound(seed0, startId) : blank(startId))));
  const [greenies, setGreenies] = useState(() => (initial ? (initial.greenies ? { ...initial.greenies } : {}) : (seed0 && seed0.greenies ? { ...seed0.greenies } : {})));
  const mounted = useRef(false);
  useEffect(() => { if (!mounted.current) { mounted.current = true; return; } if (initial) { setHs(blank(courseId)); setGreenies({}); setDate(defaultDate(courseId)); return; } const e = existingFor(courseId); setHs(e ? fromRound(e, courseId) : blank(courseId)); setGreenies(e && e.greenies ? { ...e.greenies } : {}); setDate(e ? e.date : defaultDate(courseId)); /* eslint-disable-next-line */ }, [courseId]);
  useEffect(() => { if (initial || !onLive) return; const hasScore = PLAYERS.some((p) => (hs[p.name] || []).some((x) => x !== "")); if (!hasScore) return; const holeScores = {}; PLAYERS.forEach((p) => (holeScores[p.name] = hs[p.name].map((x) => (x === "" ? null : Number(x))))); const g = {}; Object.keys(greenies).forEach((k) => { if (greenies[k]) g[k] = greenies[k]; }); const ex = existingFor(courseId); onLive({ id: ex ? ex.id : "r-" + courseId, courseId, date, holeScores, greenies: g }); /* eslint-disable-next-line */ }, [date, hs, greenies]);
    ...
    <div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3">
        <div>
          ...
          <select value={courseId} onChange={(e) => setCourseId(e.target.value)} className="mt-1 w-full rounded-lg px-3 py-2 text-sm" style={{ border: `1px solid ${C.line}`, background: "#fff" }}>
            {COURSES.slice().sort((a, b) => whenKey(a) - whenKey(b)).map((c) => <option key={c.id} value={c.id}>{c.name} — par {displayPar(c.id, pars)} ({scoringHoles(c.id)}{c.id === "paynes" ? "+1" : ""})</option>)}
          </select>
          {!initial && <p className="mt-1 text-xs" style={{ color: C.muted }}>Live — scores sync to everyone as you enter them. Switch courses freely; each keeps its own card.</p>}
        </div>
        <div>
      ...
      {course.matchPlay && (
        <div className="mt-3 rounded-lg px-3 py-2 text-xs" style={{ background: "rgba(46,94,58,0.08)", color: C.ink }}>
          2v2 best-ball match · ${course.matchPlay.stake}/player
        </div>
      )}
      {!course.tiebreaker && (
      ...
      {!initial && PRIOR[courseId] && (() => {
        const grate = stk.ghost != null ? stk.ghost : 1;
        const rows = Object.keys(PRIOR[courseId]).map((name) => { const g = PRIOR[courseId][name]; let ls = 0, gs = 0, pl = 0; for (let i = 0; i < sc; i++) { const v = hs[name] && hs[name][i]; if (isNum(v) && isNum(g[i])) { ls += Number(v); gs += g[i]; pl++; } } return { name, diff: ls - gs, pl }; }).filter((r) => r.pl > 0);
        if (!rows.length) return null;
        return (
          <div className="mt-2 rounded-lg px-3 py-2 text-xs" style={{ background: "rgba(201,162,75,0.12)", color: C.ink }}>
            <span className="font-semibold">{"Beat Your Ghost · $" + grate + "/stroke"}</span>
            {rows.map((r) => <span key={r.name}>{" · "}<b style={{ color: colorOf(r.name) }}>{r.name}</b> <span style={{ color: r.diff < 0 ? C.fairway : r.diff > 0 ? C.over : C.muted, fontWeight: 700 }}>{r.diff < 0 ? `${-r.diff} under` : r.diff > 0 ? `${r.diff} over` : "even"}</span></span>)}
          </div>
        );
      })()}
      ...
      {initial ? (
        <div className="mt-4 flex gap-2">
          <button onClick={save} disabled={!anyScore} className="flex-1 rounded-lg py-3 text-sm font-bold" style={{ background: anyScore ? C.fairway : "#C7C2B5", color: "#fff" }}>Save changes</button>
        ...
        <div className="mt-4 rounded-lg px-3 py-2 text-center text-xs font-semibold" style={{ background: "rgba(46,94,58,0.08)", color: C.fairway }}>✓ Saved live — everyone sees updates in real time</div>
      )}
    </div>
  );
}

function Play({ pars, rounds, stakes, isAdmin, onLive }) {
  if (!isAdmin) return <LiveView rounds={rounds} pars={pars} />;
  return (
    <div className="flex flex-col gap-5">
      <I70Strip rounds={rounds} />
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        <RoundEditor pars={pars} stakes={stakes} rounds={rounds} onLive={onLive} />
      </section>
    </div>
  );
}

function LiveView({ rounds, pars }) {
  const played = COURSES.slice().sort((a, b) => whenKey(a) - whenKey(b));
  const withR = played.filter((c) => rounds.some((r) => r.courseId === c.id));
  const [courseId, setCourseId] = useState((withR[0] || played[0]).id);
  const round = rounds.find((r) => r.courseId === courseId);
  const holePars = getPars(courseId, pars);
  const subtotal = (name, from, to) => { let s = 0; if (!round) return 0; const a = round.holeScores[name] || []; for (let i = from; i < to; i++) if (isNum(a[i])) s += Number(a[i]); return s; };
  return (
    <div className="flex flex-col gap-5">
      <I70Strip rounds={rounds} />
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        <h2 style={{ fontFamily: serif, fontSize: 20, fontWeight: 700 }}>Live scoring</h2>
        <p className="text-xs" style={{ color: C.muted }}>Read-only — updates as the admin enters each hole.</p>
        <div className="mt-3">
          <label className="block text-xs font-semibold" style={{ color: C.muted }}>COURSE</label>
          <select value={courseId} onChange={(e) => setCourseId(e.target.value)} className="mt-1 w-full rounded-lg px-3 py-2 text-sm" style={{ border: `1px solid ${C.line}`, background: "#fff" }}>
            {played.map((c) => <option key={c.id} value={c.id}>{c.name} — par {displayPar(c.id, pars)}{rounds.some((r) => r.courseId === c.id) ? "" : " (no scores yet)"}</option>)}
          </select>
        </div>
        {round ? (
          <div className="mt-4 overflow-x-auto"><Scorecard holePars={holePars} readScores={round.holeScores} subtotal={subtotal} courseId={courseId} /></div>
        ) : (
          <p className="mt-4 text-sm" style={{ color: C.muted }}>No scores entered yet for this course.</p>
        )}
      </section>
    </div>
  );
...
function Rounds({ rounds, pars, stakes, isAdmin, onUpdate, onDelete }) {
  const shown = rounds.filter((r) => PLAYERS.some((p) => ((r.holeScores && r.holeScores[p.name]) || []).some(isNum)));
  return (
    <div className="flex flex-col gap-3">
      <h2 className="mb-1" style={{ fontFamily: serif, fontSize: 20, fontWeight: 700 }}>Rounds <span style={{ color: C.muted, fontWeight: 400 }}>({shown.length})</span></h2>
      {shown.length === 0 ? <Empty text="No rounds yet. Head to Play to log your first round hole by hole." />
        : <div className="flex flex-col gap-3">{[...shown].reverse().map((r) => <RoundCard key={r.id} round={r} pars={pars} stakes={stakes} isAdmin={isAdmin} onUpdate={onUpdate} onDelete={onDelete} />)}</div>}
    </div>
  );
}

function RoundCard({ round, pars, stakes, isAdmin, onUpdate, onDelete }) {
        ...
        <div className="mt-3 flex flex-col gap-4">
          ...
          {course.tiebreaker ? (
            ...
            <>
              ...
              {course.id === "twin" && (
                <Detail title="Doug Adams Tax" right={`$${stakes.dat}/stroke over par`}>
                  <div className="flex flex-col gap-1">
                    {PLAYERS.map((p) => {
...
const EVENTS = [
  ...
  { date: "2026-09-03", time: "7:00 PM", kind: "Football", title: "Mizzou vs Arkansas-Pine Bluff", sub: "Kickoff · SEC Network" },
  { date: "2026-09-04", time: "7:00 PM", kind: "Football", title: "KU vs Long Island", sub: "Kickoff · ESPNU" },
  { date: "2026-09-05", time: "6:30 PM", kind: "Football", title: "Clemson at LSU", sub: "Kickoff · ABC · 7:30 PM ET" },
...
function Schedule() {
  ...
  const tone = { Golf: C.fairway, Dinner: C.gold, Lodging: C.pine, Event: C.gold, Football: "#8A5A2B" };
  return (
...
function Courses({ pars, isAdmin, setPars }) {
...
function Stats({ rounds, pars }) {
  ...
  const ps = who === "all" ? null : playerStats(rounds, pars, who);
  const pcolor = (PLAYERS.find((p) => p.name === who) || {}).color;
  return (
    <div className="flex flex-col gap-6">
      <div>
        <label className="block text-xs font-semibold" style={{ color: C.muted }}>VIEW</label>
        <div style={{ position: "relative", maxWidth: 240 }}>
          <select value={who} onChange={(e) => setWho(e.target.value)} className="mt-1 w-full appearance-none rounded-lg pl-3 pr-9 py-2 text-sm" style={{ border: `1px solid ${C.line}`, background: "#fff", color: C.ink }}>
      ...
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        <div className="flex items-center gap-2">
          <span style={{ display: "inline-block", width: 12, height: 12, borderRadius: 99, background: pcolor }} />
          <h2 style={{ fontFamily: serif, fontSize: 20, fontWeight: 700, color: pcolor }}>{who}</h2>
        </div>
        <p className="text-xs" style={{ color: C.muted }}>Scored rounds only — putting tiebreakers excluded.</p>
        <div className="mt-3 grid grid-cols-2 gap-2">
          {[{ l: "Rounds", v: String(ps.n), c: C.ink }, { l: "Round wins", v: String(ps.wins), c: C.ink }, { l: "Scoring avg", v: ps.n ? toParStr(Math.round(ps.avg * 10) / 10) : "—", c: ps.n ? parColor(Math.round(ps.avg * 10) / 10) : C.ink }, { l: "Total to par", v: ps.n ? toParStr(ps.totalToPar) : "—", c: ps.n ? parColor(ps.totalToPar) : C.ink, sub: ps.n ? `par ${ps.totalPar}` : null }].map((s) => (
            <div key={s.l} className="rounded-lg px-3 py-2" style={{ border: `1px solid ${C.line}` }}>
              <div className="text-xs" style={{ color: C.muted }}>{s.l}</div>
              <div className="text-lg font-bold" style={{ fontFamily: serif, color: s.c }}>{s.v}</div>
              {s.sub ? <div className="text-xs" style={{ color: C.muted }}>{s.sub}</div> : null}
            </div>
          ))}
        </div>
        {ps.n > 0 && (
          <div className="mt-3 flex flex-col gap-2">
            <div className="flex items-center justify-between rounded-lg px-3 py-2" style={{ border: `1px solid ${C.line}` }}><span className="text-sm" style={{ color: C.muted }}>Best round</span><span className="text-sm font-bold">{ps.best.name} <span style={{ color: parColor(ps.best.toPar) }}>({toParStr(ps.best.toPar)})</span></span></div>
            <div className="flex items-center justify-between rounded-lg px-3 py-2" style={{ border: `1px solid ${C.line}` }}><span className="text-sm" style={{ color: C.muted }}>Toughest round</span><span className="text-sm font-bold">{ps.worst.name} <span style={{ color: parColor(ps.worst.toPar) }}>({toParStr(ps.worst.toPar)})</span></span></div>
          </div>
        )}
      </section>

      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        <h2 style={{ fontFamily: serif, fontSize: 20, fontWeight: 700 }}>Scoring shapes</h2>
        <p className="text-xs" style={{ color: C.muted }}>Results by hole across every round.</p>
        <div className="mt-3 grid grid-cols-3 gap-2">
          {[["eagle", "Eagle+"], ["birdie", "Birdie"], ["par", "Par"], ["bogey", "Bogey"], ["double", "Double"], ["triple", "Triple+"]].map(([k, l]) => (
            <div key={k} className="rounded-lg px-3 py-2 text-center" style={{ border: `1px solid ${C.line}` }}>
              <div className="text-xl font-bold" style={{ fontFamily: serif, color: C.ink }}>{d.cats[who][k]}</div>
      ...
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        <h2 style={{ fontFamily: serif, fontSize: 20, fontWeight: 700 }}>By course</h2>
        <p className="text-xs" style={{ color: C.muted }}>Average score to par on each course played.</p>
        <div className="mt-3 flex flex-col gap-2">
          {ps.courses.length ? ps.courses.map((c) => (
            <div key={c.id} className="flex items-center justify-between rounded-lg px-3 py-2" style={{ border: `1px solid ${C.line}` }}>
              <span className="text-sm">{c.name}</span>
              <span className="text-sm font-bold" style={{ color: parColor(Math.round(c.avg * 10) / 10) }}>{toParStr(Math.round(c.avg * 10) / 10)}</span>
            </div>
          )) : <p className="text-xs" style={{ color: C.muted }}>No scored rounds yet.</p>}
        </div>
      </section>
        </>
      )}
  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        ...
        <p className="text-xs" style={{ color: C.muted }}>{isAdmin ? "Set the wager for each game. Changes apply to every round and the money board instantly." : "Wager amounts for each game, set by the admin."}</p>
          ...
          {ROWS.map(([k, label]) => (
            <div key={k}>
              ...
              {isAdmin ? (
                ...
                <div className="mt-1 rounded-lg px-3 py-2 text-sm font-bold" style={{ border: `1px solid ${C.line}`, background: "rgba(0,0,0,0.02)", color: C.ink }}>${stakes[k]}</div>
              )}
            </div>
          ))}
        </div>
...
function playerStats(allRounds, pars, name) {
  const rounds = allRounds.filter((r) => !courseById(r.courseId).tiebreaker && r.holeScores);
  const perRound = [];
  const roundParFor = (r) => { const rp = getPars(r.courseId, pars); const sc = scoringHoles(r.courseId); const arr = (r.holeScores && r.holeScores[name]) || []; let p = 0; for (let i = 0; i < sc; i++) if (isNum(arr[i])) p += rp[i]; return p; };
  rounds.forEach((r) => { const tp = roundToPar(r, name, pars); if (tp != null) perRound.push({ id: r.courseId, name: courseById(r.courseId).name, toPar: tp, par: roundParFor(r), win: roundWinner(r) === name }); });
  const n = perRound.length;
  const totalToPar = perRound.reduce((a, b) => a + b.toPar, 0);
  const totalPar = perRound.reduce((a, b) => a + b.par, 0);
  const wins = perRound.filter((x) => x.win).length;
  const avg = n ? totalToPar / n : 0;
  const sorted = perRound.slice().sort((a, b) => a.toPar - b.toPar);
  const best = sorted[0] || null; const worst = sorted[sorted.length - 1] || null;
  const byCourse = {}; perRound.forEach((x) => { (byCourse[x.id] = byCourse[x.id] || []).push(x.toPar); });
  const courses = Object.keys(byCourse).map((id) => ({ id, name: courseById(id).name, count: byCourse[id].length, avg: byCourse[id].reduce((a, b) => a + b, 0) / byCourse[id].length }));
  return { n, totalToPar, totalPar, wins, avg, best, worst, courses };
}

function Ghost({ rounds, stakes, isAdmin, onPick }) {
  const rate = stakes && stakes.ghost != null ? stakes.ghost : 1;
  const gr = ghostRace(rounds, rate);
  if (!gr.byCourse.length) return <Empty text="No ghost courses set up yet." />;
  const house = gr.houseName;
  const houseNet = house ? gr.direct[house] : 0;
  const FULL18 = ["ozarks", "buffalo", "paynes"];
  const usedFull = {}; gr.byCourse.forEach((c) => { if (FULL18.indexOf(c.courseId) >= 0 && c.pick) usedFull[c.courseId] = c.pick; });
  const scored = (cid) => { const rd = rounds.find((r) => r.courseId === cid); return !!(rd && PLAYERS.some((p) => ((rd.holeScores && rd.holeScores[p.name]) || []).some(isNum))); };
  const disabledFor = (cid) => { const s = {}; if (FULL18.indexOf(cid) < 0) return s; Object.keys(usedFull).forEach((k) => { if (k !== cid) s[usedFull[k]] = true; }); return s; };
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        <h2 style={{ fontFamily: serif, fontSize: 20, fontWeight: 700 }}>Beat Your Ghost</h2>
        <p className="text-xs" style={{ color: C.muted }}>Each man races his {PRIOR_YEAR} card at {money(rate)}/stroke. {house || "The odd man"} plays the House: each round he fades one golfer — that man beats his ghost and {house} pays, ties or does worse and he pays {house}. The other two only owe if they miss — a tie or worse feeds that day’s putting pool.</p>
        <div className="mt-3 rounded-lg px-3 py-3 text-center" style={{ background: houseNet > 0 ? "rgba(46,94,58,0.08)" : houseNet < 0 ? "rgba(178,34,34,0.08)" : "rgba(111,106,94,0.08)", border: `1px solid ${C.line}` }}>
          <div className="text-xs font-semibold" style={{ color: C.muted, letterSpacing: "0.08em" }}>THE HOUSE</div>
          <div className="text-lg font-bold" style={{ fontFamily: serif, color: house ? colorOf(house) : C.ink }}>{house || "—"}</div>
          <div className="text-sm font-bold" style={{ color: netColor(houseNet) }}>{houseNet > 0 ? `up ${money(houseNet)} on his picks` : houseNet < 0 ? `down ${money(-houseNet)} on his picks` : "even on his picks"}</div>
        </div>
      </section>
      {gr.byCourse.map((c) => {
        const dis = disabledFor(c.courseId);
        const locked = scored(c.courseId);
        return (
          <section key={c.courseId} className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
            <div className="flex items-baseline justify-between">
              <h2 style={{ fontFamily: serif, fontSize: 18, fontWeight: 700 }}>{c.name}</h2>
              <span className="text-xs" style={{ color: C.muted }}>misses feed {c.poolName}</span>
            </div>
            <div className="mt-2 rounded-lg px-3 py-2" style={{ background: "rgba(201,162,75,0.10)", border: `1px solid ${C.line}` }}>
              <div className="text-xs font-semibold" style={{ color: C.ink }}>{house}’s pick {FULL18.indexOf(c.courseId) >= 0 ? "(no repeats across the 18s)" : "(free pick)"}</div>
              {isAdmin && !locked ? (
                <div className="mt-1 flex flex-wrap gap-2">
                  {Object.keys(PRIOR[c.courseId]).map((n) => (
                    <button key={n} disabled={!!dis[n]} onClick={() => onPick && onPick(c.courseId, c.pick === n ? null : n)} className="rounded-lg px-3 py-1.5 text-xs font-bold" style={{ border: `1px solid ${c.pick === n ? colorOf(n) : C.line}`, color: dis[n] ? C.line : colorOf(n), background: c.pick === n ? "rgba(201,162,75,0.20)" : "#fff", opacity: dis[n] ? 0.55 : 1 }}>{n}{c.pick === n ? " ✓" : ""}</button>
                  ))}
                </div>
              ) : (
                <div className="mt-1 text-sm font-bold" style={{ color: c.pick ? colorOf(c.pick) : C.muted }}>{c.pick || "— not set"}{locked && c.pick ? " (locked)" : ""}</div>
              )}
            </div>
            {c.dormant ? <p className="mt-2 text-xs" style={{ color: C.muted }}>Dormant — no money until {house} makes a pick.</p> : null}
            <div className="mt-2 flex flex-col gap-2">
              {c.rows.map((r) => {
                const d = r.diff;
                const tag = r.played === 0 ? "not started" : d < 0 ? `${-d} under` : d > 0 ? `${d} over` : r.complete ? "tied" : "even";
                let note = "";
                if (!c.dormant && r.played > 0) {
                  if (r.isPick) note = r.amt > 0 ? `${money(r.amt)} from ${house}` : r.amt < 0 ? `${money(-r.amt)} to ${house}` : "";
                  else note = r.amt < 0 ? `${money(-r.amt)} → ${c.poolName}` : (r.status === "beat" ? "safe" : "");
                }
                return (
                  <div key={r.name} className="rounded-lg px-3 py-2" style={{ border: `1px solid ${r.isPick ? C.gold : C.line}`, background: r.isPick ? "rgba(201,162,75,0.06)" : "transparent" }}>
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-bold" style={{ color: colorOf(r.name) }}>{r.name}{r.isPick ? <span style={{ color: C.muted, fontWeight: 400 }}> · the pick</span> : null}</span>
                      <span className="text-sm font-bold" style={{ color: r.amt > 0 ? C.fairway : r.amt < 0 ? C.over : C.muted }}>{note || (r.played === 0 ? "—" : "")}</span>
                    </div>
                    <div className="mt-0.5 flex items-center justify-between text-xs">
                      <span style={{ color: C.muted }}>Ghost {r.ghost} · Live {r.live}{r.played > 0 && !r.complete ? ` · thru ${r.played}` : ""}</span>
                      <span style={{ color: d < 0 ? C.fairway : d > 0 ? C.over : C.muted, fontWeight: 700 }}>{r.played === 0 ? "" : tag}</span>
                    </div>
                  </div>
                );
              })}
...
/* ============ TRIP ============ */
function Trip({ rounds, pars, stakes, isAdmin, setStakes, onClear, onPuttOff }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-2xl p-5" style={{ background: C.pine, color: C.parchment }}>
        ...
        {!champ.overall ? <p className="mt-2 text-sm" style={{ color: "rgba(243,238,227,0.8)" }}>Log rounds to crown a champion.</p> : (
          <>
            ...
            <p className="text-sm" style={{ color: "rgba(243,238,227,0.85)" }}>{toParStr(champ.overall.total)} to par · {champ.overall.rounds} rounds · low man wins</p>
      ...
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
        ...
        <p className="text-xs" style={{ color: C.muted }}>Net across {hb.length} scored round{hb.length === 1 ? "" : "s"}, including the I-70 Cup (live regional standing) and Beat Your Ghost (Brian banks it). Each game = every player antes the stake, the winner takes the pot.</p>
        <div className="mt-3 overflow-x-auto">
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 720 }}>
            <thead><tr style={{ color: C.muted }}>{["Player", "Skins", "Nassau", "Greenies", "Match", "Tiebreak", "Ghost", "I-70 Cup", "Net"].map((h) => <th key={h} className="px-2 py-1 text-left text-xs">{h}</th>)}</tr></thead>
            <tbody>
              {moneyRows.map((r, i) => (
                <tr key={r.name} style={{ borderTop: `1px solid ${C.line}`, background: i === 0 && r.total > 0 ? "rgba(201,162,75,0.12)" : "transparent" }}>
                  ...
                  <td className="px-2 py-2 text-sm" style={{ color: netColor(r.ghost) }}>{money(r.ghost)}</td>
            ...
            return (
              <div key={tb.courseId} className="rounded-xl p-3" style={{ border: `1px solid ${C.line}` }}>
                ...
                {tb.played && order.length > 0 && (
                  <div className="mt-2 grid grid-cols-4 gap-2">
                    {order.map((r, i) => (
                      <div key={r.name} className="rounded-lg py-2 text-center" style={{ background: r.name === tb.winner ? "rgba(201,162,75,0.16)" : "transparent", border: `1px solid ${r.name === tb.winner ? C.gold : C.line}` }}>
                ...
                <div className="mt-2 rounded-lg px-3 py-2 text-sm" style={{ background: tb.winner ? "rgba(46,94,58,0.08)" : "rgba(201,162,75,0.1)", color: C.ink }}>
                  {(mine.length === 0 && taxPool === 0)
                    ...
                    : tb.winner
                      ? <span><b style={{ color: colorOf(tb.winner) }}>{tb.winner}</b> won{tb.puttOff ? " a putt-off" : ""} and swept {money(fullWon)}{taxPool > 0 ? " (pushes + Doug Adams Tax)" : ""}.</span>
                      : tb.tie
                        ? `Putting is tied — ${money(fullPot)} rides on a putt-off.`
                        : <span><b>{money(fullPot)}</b> waiting on this course — lowest total sweeps it.</span>}
                </div>
                {isAdmin && tb.tie && fullPot > 0 && (
                  <div className="mt-2 rounded-lg px-3 py-2" style={{ border: `1px dashed ${C.gold}`, background: "rgba(201,162,75,0.06)" }}>
                    <div className="text-xs font-semibold" style={{ color: C.ink }}>Putt-off — declare the winner</div>
                    <div className="mt-1 flex flex-wrap gap-2">
                      {tb.tiedNames.map((n) => <button key={n} onClick={() => onPuttOff && onPuttOff(tb.courseId, n)} className="rounded-lg px-3 py-1.5 text-xs font-bold" style={{ border: `1px solid ${colorOf(n)}`, color: colorOf(n), background: "#fff" }}>{n} wins</button>)}
                    </div>
                  </div>
                )}
                {isAdmin && tb.puttOff && (
                  <div className="mt-2 flex items-center justify-between text-xs">
                    <span style={{ color: C.muted }}>Winner decided by putt-off.</span>
                    <button onClick={() => onPuttOff && onPuttOff(tb.courseId, null)} className="font-semibold" style={{ color: C.muted }}>Undo</button>
                  </div>
                )}
                {mine.length > 0 && (
                  <div className="mt-2 flex flex-col gap-1">
      ...
      {isAdmin && <button onClick={() => { if (window.confirm("Erase all logged rounds? This can't be undone.")) onClear(); }} className="self-center rounded-lg px-4 py-2 text-xs font-semibold" style={{ color: C.over, border: `1px solid ${C.line}`, background: C.card }}>Reset all rounds</button>}
    </div>
  );
}
...
function tripChampion(allRounds, pars, tally) {
  ...
  const avgs = PLAYERS.map((p) => ({ name: p.name, rounds: t[p.name].length, total: t[p.name].reduce((x, y) => x + y, 0), avg: t[p.name].length ? t[p.name].reduce((x, y) => x + y, 0) / t[p.name].length : Infinity })).filter((x) => x.rounds > 0);
  const overall = avgs.length ? avgs.slice().sort((a, b) => a.total - b.total)[0] : null;
