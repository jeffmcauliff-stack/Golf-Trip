...
const DEFAULT_STAKES = { skin: 2, nassauFront: 10, nassauBack: 10, nassauTotal: 10, greenie: 5, tiger: 15, i70: 35, dat: 1 };
    ...
    <div style={{ background: C.parchment, minHeight: "100vh", color: C.ink, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <main className="mx-auto px-4 pb-20 pt-4" style={{ maxWidth: 820 }}>
        {loading ? <p className="py-12 text-center" style={{ color: C.muted }}>Loading the scorecard…</p>
          : showTab === "play" ? <Play pars={pars} rounds={sorted} stakes={stakes} isAdmin={isAdmin} onLive={(r) => save(K_ROUNDS, upsertRound(rounds, r), setRounds)} />
          : showTab === "rounds" ? <Rounds rounds={sorted} pars={pars} stakes={stakes} isAdmin={isAdmin} onUpdate={(id, r) => save(K_ROUNDS, rounds.map((x) => (x.id === id ? { ...r, id } : x)), setRounds)} onDelete={(id) => save(K_ROUNDS, rounds.filter((x) => x.id !== id), setRounds)} />
          : showTab === "stats" ? <Stats rounds={sorted} pars={pars} />
          : showTab === "trip" ? <Trip rounds={sorted} pars={pars} stakes={stakes} isAdmin={isAdmin} setStakes={(v) => save(K_STAKES, v, setStakes)} onClear={() => save(K_ROUNDS, [], setRounds)} />
...
function Tabs({ tab, setTab, isAdmin }) {
  const base = [["play", "Play"], ["rounds", "Rounds"], ["trip", "Leaderboard"], ["stats", "Stats"], ["schedule", "Schedule"], ["courses", "Courses"], ["history", "History"], ["stakes", "Stakes"]];
  const items = base;
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {rounds.length === 0 ? <Empty text="No rounds yet. Head to Play to log your first round hole by hole." />
        : <div className="flex flex-col gap-3">{[...rounds].reverse().map((r) => <RoundCard key={r.id} round={r} pars={pars} stakes={stakes} isAdmin={isAdmin} onUpdate={onUpdate} onDelete={onDelete} />)}</div>}
    </div>
  );
}
...
function Stakes({ stakes, isAdmin, setStakes }) {
  const ROWS = [["skin", "Skin / hole"], ["nassauFront", "Nassau front"], ["nassauBack", "Nassau back"], ["nassauTotal", "Nassau total"], ["greenie", "Greenie each"], ["tiger", "★ Tiger Greenie"], ["i70", "I-70 Cup / player"], ["dat", "Doug Adams Tax / stroke over par"]];
  return (
    <div className="flex flex-col gap-6">
      <section className="rounded-2xl p-4" style={{ background: C.card, border: `1px solid ${C.line}` }}>
...
/* ============ TRIP ============ */
function Trip({ rounds, pars, stakes, isAdmin, setStakes, onClear }) {
